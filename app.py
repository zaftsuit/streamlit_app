...
if st.button("Predict"):
    ...
    surv_funcs = Gboost_cox.predict_survival_function(X)
    cumulative_hazard=Gboost_cox.predict_cumulative_hazard_function(X)
    # Output prediction
    st.header('outcome prediction')
    st.text(f"mortality risk:{prediction}")
    st.header('survival function accumulated over time')
    st.table(surv_funcs)
    st.header('The hazard function accumulated over time')
    st.table(cumulative_hazard)
